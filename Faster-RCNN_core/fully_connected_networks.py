...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_reshaped = x.reshape(N, -1)  # Reshape input to (N, D), where D = d_1 * ... * d_k
        out = torch.mm(x_reshaped, w) + b  # Perform the linear operation
    ...
    def backward(dout, cache):
        ...
        dx = torch.mm(dout, w.t()).view_as(x)  
...
def svm_loss(x, y):
    ...
    correct_class_scores = x[torch.arange(N), y].view(-1, 1)
    
    # Calculate margins
    margins = torch.maximum(torch.zeros_like(x), x - correct_class_scores + 1)
    margins[torch.arange(N), y] = 0  # Do not consider correct classes in the margin calculation
    
    # Calculate loss
    loss = torch.sum(margins) / N
    
    # Calculate gradient
    positive_margins = (margins > 0).float()
    count_positive = torch.sum(positive_margins, axis=1) - 1  # Subtract 1 to not count the correct class
    dx = positive_margins
    dx[torch.arange(N), y] -= count_positive  # Subtract from the correct class scores
...
def softmax_loss(x, y):
    ...
    shifted_scores = x - x.max(dim=1, keepdim=True).values
    
    softmax_probs = torch.exp(shifted_scores) / torch.sum(torch.exp(shifted_scores), dim=1, keepdim=True)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Forward pass
        hidden_layer = torch.relu(X.mm(W1) + b1)
        ...
        softmax_grad = softmax_loss(scores, y)[1]

        # Gradients for parameters of the second layer
        grads['W2'] = hidden_layer.t().mm(softmax_grad) + 2 * self.reg * W2
        grads['b2'] = softmax_grad.sum(dim=0)

        # Backpropagate the gradient through the relu
        hidden_grad = softmax_grad.mm(W2.t())
        hidden_grad[hidden_layer <= 0] = 0

        # Gradients for parameters of the first layer
        grads['W1'] = X.t().mm(hidden_grad) + 2 * self.reg * W1
        grads['b1'] = hidden_grad.sum(dim=0)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            scores, cache = Linear_ReLU.forward(scores, self.params[f'W{i}'], self.params[f'b{i}'])
            cache_history.append(cache)
            if self.use_dropout:
                scores, dropout_cache_i = Dropout.forward(scores, self.dropout_param)
                dropout_cache.append(dropout_cache_i)
        ...
        # Backward pass
        dscores, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, cache_history.pop())
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']

        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dscores = Dropout.backward(dscores, dropout_cache.pop())
            dscores, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dscores, cache_history.pop())
...
def create_solver_instance(data_dict, dtype, device, **kwargs):
    ...
    update_rules = {
        'sgd': Solver.sgd,
        # 'adam': Solver.adam,  # Uncomment or add this if you have an Adam update function defined in Solver
    }

    # Select the update rule based on a string identifier provided in kwargs or default to 'sgd'
    update_rule_name = kwargs.pop('update_rule', 'sgd')
    update_rule_function = update_rules.get(update_rule_name, Solver.sgd)

    solver_kwargs = {
        'update_rule': update_rule_function,
        'optim_config': {
            'learning_rate': 1e-5,
        },
        'lr_decay': 0.9,
        'num_epochs': 10,  # Adjust as necessary for adequate training
        'batch_size': 100,
        'print_every': 100,
        'device': device
    }

    # Update solver_kwargs with any additional kwargs provided
    solver_kwargs.update(kwargs)

    # Create a Solver instance with the model, data, and solver configuration
    solver = Solver(model=model, data=data_dict, **solver_kwargs)
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 5e-6   # Experiment with this!
    learning_rate = 10  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                             END OF YOUR CODE                 #
...
def rmsprop(w, dw, config=None):
    ...
    # Update the cache with the moving average of squared gradients
    config['cache'] = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * (dw ** 2)
    
    # Update the weights using the RMSProp update rule
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            mask = (torch.rand(x.shape, device=x.device) > p) / (1 - p) # Scale activations up during training time
            ...
            out = x  # During testing, dropout is not applied, so just return the input
