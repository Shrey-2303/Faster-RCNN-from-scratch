...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        p3_layer = self.fpn_params['conv1_c3'](backbone_feats['c3'])
        p4_layer = self.fpn_params['conv1_c4'](backbone_feats['c4'])
        p5_layer = self.fpn_params['conv1_c5'](backbone_feats['c5'])
        p4_layer = p4_layer + F.interpolate(p5_layer, scale_factor=scale_4_5)
        p3_layer = p3_layer + F.interpolate(p4_layer, scale_factor=scale_3_4)
        fpn_feats['p3'] = self.fpn_params['conv3_c3'](p3_layer)
        fpn_feats['p4'] = self.fpn_params['conv3_c4'](p4_layer)
        fpn_feats['p5'] = self.fpn_params['conv3_c5'](p5_layer)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x_coords = (torch.arange(feat_shape[3], dtype=dtype, device=device) + 0.5) * level_stride
        y_coords = (torch.arange(feat_shape[2], dtype=dtype, device=device) + 0.5) * level_stride
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xmin_inter = torch.max(boxes1[:, 0].unsqueeze(1), boxes2[:, 0].unsqueeze(0))
    ymin_inter = torch.max(boxes1[:, 1].unsqueeze(1), boxes2[:, 1].unsqueeze(0))
    xmax_inter = torch.min(boxes1[:, 2].unsqueeze(1), boxes2[:, 2].unsqueeze(0))
    ymax_inter = torch.min(boxes1[:, 3].unsqueeze(1), boxes2[:, 3].unsqueeze(0))

    inter_area = torch.clamp(xmax_inter - xmin_inter, min=0) * torch.clamp(ymax_inter - ymin_inter, min=0)
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    keep = torch.zeros(scores.size(0), dtype=torch.long, device=boxes.device)
    num_keep = 0

    while boxes.size(0) > 0:
        keep[num_keep] = indices[0]
        num_keep += 1

        if boxes.size(0) == 1: break

        ious = iou(boxes[0].unsqueeze(0), boxes[1:]).squeeze()
