...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    flattened_train = x_train.reshape(num_train,-1)
    flattened_test = x_test.reshape(num_test,-1)
    for i in range(num_train):
      for j in range(num_test):
        # Flatten the images and compute squared Euclidean distance
        dists[i, j] = torch.sum((flattened_train[i] - flattened_test[j])**2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    flattened_test = x_test.reshape(num_test,-1)
    for i in range(num_train):
      dists[i,:]=torch.sum((flattened_train[i] - flattened_test)**2,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Reshape tensors to flatten images
    flattened_train = x_train.reshape(num_train,-1)
    flattened_test = x_test.reshape(num_test,-1)

    # Compute squared Euclidean distance using broadcasting and matrix operations
    dists = torch.sum(flattened_train**2, dim=1, keepdim=True) \
          + torch.sum(flattened_test**2, dim=1) \
          - 2 * torch.mm(flattened_train, flattened_test.T)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        # Use all but the i-th fold as training data
        ...
        # Use the i-th fold as validation set
        x_val_fold = x_train_folds[i]
        y_val_fold = y_train_folds[i]

        # Create and train KnnClassifier
        knn_classifier = KnnClassifier(x_train_fold, y_train_fold)

        # Make predictions on validation set
        y_val_pred = knn_classifier.predict(x_val_fold, k=k)

        # Check accuracy
        accuracies.append(knn_classifier.check_accuracy(x_val_fold, y_val_fold))
