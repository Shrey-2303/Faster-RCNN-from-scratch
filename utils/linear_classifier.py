...
def svm_loss_vectorized(
    ...
    margins = torch.maximum(torch.tensor(0.0), scores - correct_class_scores + 1)
    ...
    # Compute gradient
    margins[margins > 0] = 1
    valid_margin_count = torch.sum(margins, axis=1)
    margins[torch.arange(num_train), y] -= valid_margin_count
...
def svm_get_search_params():
    ...
    learning_rates = [1e-7, 5e-5, 1e-4, 5e-4, 1e-3]
    ...
    regularization_strengths = [1e-4, 1e-2, 1e0, 1e2, 1e4]
...
def test_one_param_set(
    ...
    # Predict labels on training set and calculate accuracy
    y_train_pred = cls.predict(data_dict['X_train'])
    train_acc = (y_train_pred == data_dict['y_train']).float().mean().item()

    # Predict labels on validation set and calculate accuracy
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        # vector of scores
        f_i = X[i].unsqueeze(0).mm(W)

        # Normalization trick to avoid numerical instability, subtract max from scores
        f_i -= torch.max(f_i)

        # loss for the i-th example
        sum_j = torch.sum(torch.exp(f_i))
        p = lambda k: torch.exp(f_i[0, k]) / sum_j
        loss += -torch.log(p(y[i]))
        # Compute gradient
        for k in range(num_classes):
            p_k = p(k)
            dW[:, k] += (p_k - (k == y[i]).float()) * X[i]
...
def softmax_loss_vectorized(
    ...
    softmax_probs = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1, keepdim=True)

    # Compute loss
    correct_class_probs = softmax_probs[torch.arange(X.shape[0]), y]
    loss = -torch.sum(torch.log(correct_class_probs)) / X.shape[0]
    loss += reg * torch.sum(W * W)
...
def softmax_get_search_params():
    ...
    learning_rates = [ 1e-2, 0.1, 1, 10, 100]
