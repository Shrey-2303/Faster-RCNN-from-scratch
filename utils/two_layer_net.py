...
import random
import statistics
from rob599.p1_helpers import sample_batch
...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_pre_activation = X.mm(W1) + b1 # N x H
    hidden = hidden_pre_activation.clamp(min=0)  # N x H 
    scores = hidden.mm(W2) + b2 # N x C
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    softmax = torch.exp(scores).div(torch.exp(scores).sum(dim=1, keepdim=True))
    loss = -torch.log(softmax[torch.arange(N), y]).mean() + reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2))
...
def nn_get_search_params():
    ...
    learning_rates = [1e-2, 0.1, 1, 10]
    hidden_sizes = [50, 100, 200, 300,]
    regularization_strengths = [1e-6, 1e-5, 1e-4]
    learning_rate_decays = [0.94, 0.95, 0.98, 1]
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    ...
                    net = TwoLayerNet(input_size=X_train.shape[1], hidden_size=hs, output_size=y_train.max() + 1)
                    stat = net.train(X_train, y_train, X_val, y_val, learning_rate=lr, learning_rate_decay=lrd, reg=reg)
